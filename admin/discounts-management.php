...
function handleGetStats($db) {
    try {
        // Get total discounts
        $totalResult = $db->rawQuery("SELECT COUNT(*) as total FROM product_discounts");
        $total = intval($totalResult[0]['total'] ?? 0);
        
        // Get active discounts (active and within date range)
        $activeResult = $db->rawQuery("SELECT COUNT(*) as active FROM product_discounts WHERE is_active = 1 AND start_date <= CURDATE() AND end_date >= CURDATE()");
        $active = intval($activeResult[0]['active'] ?? 0);
        
        // Get upcoming discounts (active but start date is in future)
        $upcomingResult = $db->rawQuery("SELECT COUNT(*) as upcoming FROM product_discounts WHERE is_active = 1 AND start_date > CURDATE()");
        $upcoming = intval($upcomingResult[0]['upcoming'] ?? 0);
        
        // Get expired discounts (end date has passed)
        $expiredResult = $db->rawQuery("SELECT COUNT(*) as expired FROM product_discounts WHERE end_date < CURDATE()");
        $expired = intval($expiredResult[0]['expired'] ?? 0);
...
<body class="sb-nav-fixed">
    ...
    $(document).ready(function() {
        ...
        $(document).on('click', '.toggle-discount', function() {
            ...
            console.log('Toggling discount:', discountId, 'status:', status);
            ...
            $.ajax({
                ...
                error: function(xhr, status, error) {
                    console.error('Toggle AJAX Error:', xhr.responseText);
                    Swal.fire({
                        icon: 'error',
                        title: 'Error!',
