...
if ($category_id) {
    ...
    $lookupDb = new MysqliDb();
    $lookupDb->where('id', $category_id);
    $lookupDb->where('is_active', 1);
    $category = $lookupDb->getOne('categories');
    $category_name = $category ? $category['name'] : null;
}

// Subcategory filter
if ($subcategory_id) {
    // Apply to products query only
    $db->where('subcategory_id', $subcategory_id);

    // Use a separate DB instance to fetch subcategory name to avoid leaking product filters
    $lookupDb = new MysqliDb();
    $lookupDb->where('id', $subcategory_id);
    $lookupDb->where('is_active', 1);
    $subcategory = $lookupDb->getOne('subcategories');
...
$countDb = $db->copy();
$total_products = (int) $countDb->getValue('products', 'count(*)');
